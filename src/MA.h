...
class MA : public EventBasedFilter<double, double>
{
public:
    MA();
	...
	void init(DataSeries& dataSeries, int period, int maxLen = DEFAULT_MAX_LEN);
...
class EMA : public EventBasedFilter<double, double>
{
public:
...
class SMA : public EventBasedFilter<double, double>
{
public:
    void init(DataSeries& dataSeries, int period, double weight = 1, int maxLen = DEFAULT_MAX_LEN);
...
class AMA : public EventBasedFilter<double, double>
{
public:
    AMA();
    // @param: dataSeries: The DataSeries instance being filtered.
    ...
    // @param: maxLen: The maximum number of values to hold.
    // Once a bounded length is full, when new items are added, a corresponding number of items are discarded from the opposite end.
    void init(DataSeries& dataSeries, int effRatioLen, int fastAvgLen = 2, int slowAvgLen = 30, int maxLen = DEFAULT_MAX_LEN);
